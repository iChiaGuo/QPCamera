...
#import "QPCaptureButton.h"

#define LINE_WIDTH 6.0f
#define DEFAULT_FRAME CGRectMake(0.0f, 0.0f, 68.0f, 68.0f)

@interface QPPhotoCaptureButton : QPCaptureButton
@end

@interface QPVideoCaptureButton : QPPhotoCaptureButton
@end

@interface QPCaptureButton ()
@property (strong, nonatomic) CALayer *circleLayer;
@end

@implementation QPCaptureButton

+ (instancetype)captureButton {
    return [[self alloc] initWithCaptureButtonMode:QPCaptureButtonModeVideo];
}

+ (instancetype)captureButtonWithMode:(QPCaptureButtonMode)mode {
    return [[self alloc] initWithCaptureButtonMode:mode];
}

- (id)initWithCaptureButtonMode:(QPCaptureButtonMode)mode {
...
- (void)awakeFromNib {
    [super awakeFromNib];
    _captureButtonMode = QPCaptureButtonModeVideo;
    [self setupView];
}
...
- (void)setupView {
    ...
    UIColor *circleColor = (self.captureButtonMode == QPCaptureButtonModeVideo) ? [UIColor redColor] : [UIColor whiteColor];
...
- (void)setCaptureButtonMode:(QPCaptureButtonMode)mode {
    if (_captureButtonMode != mode) {
        _captureButtonMode = mode;
        UIColor *toColor = (mode == QPCaptureButtonModeVideo) ? [UIColor redColor] : [UIColor whiteColor];
...
- (void)setSelected:(BOOL)selected {
    ...
    if (self.captureButtonMode == QPCaptureButtonModeVideo) {
