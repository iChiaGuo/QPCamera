...
- (void)setupView {
    _maxRight = YES;
    self.cameraMode = QPCameraModeVideo;
...
- (void)switchMode:(UISwipeGestureRecognizer *)recognizer {
    if (recognizer.direction == UISwipeGestureRecognizerDirectionLeft && !self.maxLeft) {
        ...
        [UIView animateWithDuration:0.28
                         ...
                         completion:^(BOOL complete){
							 self.cameraMode = QPCameraModePhoto;
                             self.maxLeft = YES;
                             self.maxRight = NO;
						 }];
...
- (void)setCameraMode:(QPCameraMode)cameraMode {
    if (_cameraMode != cameraMode) {
        _cameraMode = cameraMode;
        if (cameraMode == QPCameraModePhoto) {
            self.captureButton.selected = NO;
            self.captureButton.captureButtonMode = QPCaptureButtonModePhoto;
            self.layer.backgroundColor = [UIColor blackColor].CGColor;
        } else {
            self.captureButton.captureButtonMode = QPCaptureButtonModeVideo;
